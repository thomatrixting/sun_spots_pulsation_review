...
Or import individual functions into a notebook:
    from src.scripts import download_regions_continuum, make_continuum_cubes
"""
...
# ── All-regions download ─────────────────────────────────────────────────────

def download_regions_continuum(regions_hpc, base_dir, time_start, time_end, notify_email):
    """Search and download HMI continuum (Ic_45s) cutouts for every region."""
...
def download_regions_magnetogram(regions_hpc, base_dir, time_start, time_end, notify_email):
    """Search and download HMI magnetogram (M_45s) cutouts for every region."""
    base_dir = pathlib.Path(base_dir)
    for i, (bl, tr) in enumerate(regions_hpc):
        region_dir = base_dir / f'region_{i+1:02d}'
        region_dir.mkdir(parents=True, exist_ok=True)
...
def make_continuum_cubes(regions_hpc, base_dir):
    """Build one FITS cube per region from downloaded HMI continuum frames."""
    base_dir = pathlib.Path(base_dir)
    for i in range(1, len(regions_hpc) + 1):
        region_dir = base_dir / f'region_{i:02d}'
        make_cube(
            region_dir / 'hmi.ic_45s.20260401*.continuum.fits',
            base_dir / f'region_{i:02d}_continuum_cube.fits',
            overwrite=True,
        )


def make_magneto_cubes(regions_hpc, base_dir):
    """Build one FITS cube per region from Ic_45s frames with magnetogram suffix."""
    base_dir = pathlib.Path(base_dir)
    for i in range(1, len(regions_hpc) + 1):
        region_dir = base_dir / f'region_{i:02d}'
        make_cube(
            region_dir / 'hmi.ic_45s.20260401*.magneto*.fits',
            base_dir / f'region_{i:02d}_magneto_cube.fits',
            overwrite=True,
        )


def make_magnetogram_cubes(regions_hpc, base_dir):
    """Build one FITS cube per region from HMI M_45s magnetogram frames."""
    base_dir = pathlib.Path(base_dir)
    for i in range(1, len(regions_hpc) + 1):
        region_dir = base_dir / f'region_{i:02d}'
        make_cube(
            region_dir / 'hmi.m_45s.20260401*.magnetogram.fits',
            base_dir / f'region_{i:02d}_magnetogram_cube.fits',
            overwrite=True,
        )
...
if __name__ == "__main__":
    ...
    TIME_START = '2026-04-01T13:00:00'
    TIME_END   = '2026-04-01T17:00:00'
    ...
    download_regions_continuum(regions_hpc, BASE_DIR, TIME_START, TIME_END, NOTIFY)
    make_continuum_cubes(regions_hpc, BASE_DIR)

    download_regions_magnetogram(regions_hpc, BASE_DIR, TIME_START, TIME_END, NOTIFY)
    make_magnetogram_cubes(regions_hpc, BASE_DIR)
